...
class FullRawCorpusSearchClient:
    ...
    def from_env(cls, *, strict: bool = False) -> FullRawCorpusSearchClient:
        ...
        return cls(
            ...
            timeout=min(_float_env("V5_MEMO_FULL_RAW_CORPUS_TIMEOUT", 45.0), 45.0),
            max_variants=min(_int_env("V5_MEMO_FULL_RAW_MAX_VARIANTS", 4), 4),
    ...
    def _search_variant(self, search_pass: FullRawSearchPass, *, limit: int) -> list[CorpusHit]:
        ...
        if (
            ...
            and (initial_error is not None or sweep_status not in ("", "hit", "disabled"))
            and not (receipt and self._receipt_is_sufficient(receipt))
        ):
